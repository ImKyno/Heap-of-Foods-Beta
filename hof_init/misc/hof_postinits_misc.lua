...
-- Disabled until I figure out why some maptags can override mine 💀💀
--[[
local function SetAreaColourCubes(inst, cc_name)
	_G.TheWorld:PushEvent("overridecolourcube", cc_name and resolvefilepath("images/colourcubesimages/"..cc_name.."_cc.tex") or nil)
end
	...
	self.inst:DoTaskInTime(0.5, function()
		...
		self.inst:ListenForEvent("changearea", function(inst, area)
			if self.canchange then
				local tags = area and area.tags or {}
                
				local tag_to_cc = 
				{
					SerenityArea = "serenity",
					MeadowArea   = "meadow",
				}

				local chosen_cc = nil
				
				for tag, cc in pairs(tag_to_cc) do
				...
				SetAreaColourCubes(self.inst, chosen_cc)
			end
		end)
