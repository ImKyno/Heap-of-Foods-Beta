...
{
    numEatenFoods         = 0,
	numTradedWithPigElder = 0,
}

local bool_variables =
{
    hasBeenInSerenityBiome        = false,
    hasBeenInSeasideBiome         = false,
	hasBuiltAntChest              = false,
	hasEatenCaramelCube           = false,
	hasDrankAlcoholic             = false,
	hasDrankPirateRum             = false,
	hasDrankTequila               = false,
	hasDrankCoffee                = false,
}
...
for k,_ in pairs(allHofFoods) do
    bool_variables["hasCookedHof_" .. k] = false
end
...
for k,v in pairs(preparedHofFoods) do
    ...
    if not blacklist[k] then
        bool_variables["hasEatenHof_" .. k] = false
...
for k,v in pairs(preparedHofWarlyFoods) do
    bool_variables["hasCookedWarlyHof_" .. k] = false
end
...
for k,_ in pairs(allHofDrinks) do
	...
	if not blacklist[k] then
		bool_variables["hasBrewedHof_" .. k] = false
		edibleHofDrinks[k] = true
	end
end
...
for k,v in pairs(books) do
    bool_variables["hasCraftedHofBook_" .. k] = false
end
...
local function SetupPlayerFunctions(player)
	...
	player:ListenForEvent("learncookbookrecipe", function(inst, data)
        ...
        elseif data and type(data.product) == "string" then
...
_G[registerEntriesFuncName] = function(root)
	...
	local function GetNumCraftedBooks(data)
        return KaGetNumDone(books, function(k, v) return data["hasCraftedHofBook_" .. k] == true end)
    end
    ...
    KAACHIEVEMENT.EXTRA_TROPHY_IMAGE_PATH[categoryName] = resolvefilepath("images/achievements_images.xml")
