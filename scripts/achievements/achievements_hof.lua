...
local UpvalueHacker = require("kaachievement_utils/upvaluehacker")
...
local function IsNearPrefabs(inst, prefabList, radius)
	...
	local ents = TheSim:FindEntities(x,y,z, radius or 30)
	
	if type(prefabList) == "string" then
	...
	player:ListenForEvent("onhop", function(inst)
		player:DoTaskInTime(1, function()
			if IsOnLand(player) then
				if IsNearPrefabs(player, {"kyno_meadowisland_sandhill", "kyno_meadowisland_pond", "kyno_meadowisland_shop"}) then
...
_G[string.format("Register%sAchievements", categoryName)] = function(inst)
	if inst:HasTag("player") then
		...
		for k, v in pairs(variables) do
			assert(player.kaAchievementData[k] == nil, string.format("Variable name \"%s\" has already been used in \"player.kaAchievementData\".", k))
