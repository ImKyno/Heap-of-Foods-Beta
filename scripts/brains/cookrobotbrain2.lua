...
local function HasFoodOrder(inst)
	if inst.components.container == nil then
		print("[CookRobot] HasFoodOrder = false (no container)")
		return false
	end

	local item = inst.components.container:GetItemInSlot(1)
	print("[CookRobot] HasFoodOrder:", item and item.prefab or "nil")
	return item ~= nil
end

local function GetFoodOrder(inst)
	if inst.components.container == nil then
		print("[CookRobot] GetFoodOrder = nil (no container)")
		return nil
	end

	local item = inst.components.container:GetItemInSlot(1)
	print("[CookRobot] GetFoodOrder:", item and item.prefab or "nil")
	return item
end

local function GetOrderRecipeData(inst)
	local food = GetFoodOrder(inst)
	
	if food == nil then
		print("[CookRobot] GetOrderRecipeData: food = nil")
		return nil
    end
	...
	for cooker_type, station_prefab in pairs(TUNING.KYNO_COOK_ROBOT_COOKINGPOTS) do
		...
		if cooker_type == "cookpot" then
			...
			local recipe = recipes and recipes[food.prefab]
