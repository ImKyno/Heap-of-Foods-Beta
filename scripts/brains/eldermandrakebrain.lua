...
local SCARER_MUST_TAGS = { "eldermandrakescarer" }
local SEE_SCARER_DIST = TUNING.KYNO_ELDERMANDRAKE_SCARE_RADIUS
...
local function FindFoodAction(inst)
	...
	local target = (inst.components.inventory ~= nil and inst.components.eater ~= nil
	and inst.components.inventory:FindItem(inst._can_eat_test)) or nil
	...
	if not target then
		...
		if not time_since_eat or (time_since_eat > 2 * TUNING.PIG_MIN_POOP_PERIOD) then
			...
			target = FindEntity(inst, SEE_FOOD_DIST, function(item)
				return not item:HasTag("mandrake")
				and item.components.edible ~= nil
				and (not noveggie or item.components.edible.foodtype == FOODTYPE.ROUGHAGE)
...
local function HasValidHome(inst)
	...
	and not (home.components.burnable ~= nil and home.components.burnable:IsBurning()) and not home:HasTag("burnt")
end
...
local function GoHomeAction(inst)
	...
	if not leader and not inst.components.combat.target and HasValidHome(inst) then
...
local ElderMandrakeBrain = Class(Brain, function(self, inst)
	Brain._ctor(self, inst)
end)

local SCARER_PARAMS = 
{
	tags        = { "eldermandrakescarer" },
	notags      = { "NOCLICK" },
	seeequipped = true,
}

function ElderMandrakeBrain:OnStart()
	local root =
		PriorityNode(
		{
			BrainCommon.PanicWhenScared(self.inst, .25, "KYNO_ELDERMANDRAKE_PANICBOSS"),
			WhileNode( function() return self.inst.components.hauntable and self.inst.components.hauntable.panic end, "PanicHaunted",
				ChattyNode(self.inst, "KYNO_ELDERMANDRAKE_PANICHAUNT",
					Panic(self.inst))),
			WhileNode(function() return self.inst.components.health.takingfiredamage end, "OnFire",
				ChattyNode(self.inst, "KYNO_ELDERMANDRAKE_PANICFIRE",
					Panic(self.inst))),
			WhileNode(function() return BrainCommon.ShouldAvoidElectricFence(self.inst) end, "OnShocked",
				ChattyNode(self.inst, "KYNO_ELDERMANDRAKE_PANICELECTRICITY",
			...
			WhileNode(function() return self.inst.components.health:GetPercent() < TUNING.BUNNYMAN_PANIC_THRESH end, "LowHealth",
				ChattyNode(self.inst, "KYNO_ELDERMANDRAKE_RETREAT",
					RunAway(self.inst, "scarytoprey", SEE_PLAYER_DIST, STOP_RUN_DIST))),
			ChaseAndAttack(self.inst, MAX_CHASE_TIME, MAX_CHASE_DIST),
			ChattyNode(self.inst, "KYNO_ELDERMANDRAKE_RETREAT",
				RunAway(self.inst, SCARER_PARAMS, SEE_SCARER_DIST, STOP_SCARER_DIST)),
			...
			WhileNode(function() return TheWorld.state.isacidraining end, "IsAcidRaining",
				DoAction(self.inst, GoHomeAction, "GoHome", true), 1),
