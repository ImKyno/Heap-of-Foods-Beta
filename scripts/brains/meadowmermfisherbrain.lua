...
local MeadowMermFisherBrain = Class(Brain, function(self, inst)
...
local function GoHomeAction(inst)
	...
	return not inst.components.combat.target and home and home:IsValid() and not (home.components.burnable ~= nil
	and home.components.burnable:IsBurning()) and not home:HasTag("burnt") and BufferedAction(inst, home, ACTIONS.GOHOME)
end

local function ShouldGoHome(inst)
	local home = inst.components.homeseeker ~= nil and inst.components.homeseeker.home or nil

	return TheWorld.state.isday and home ~= nil and (home.components.childspawner == nil
...
local function Fish(inst)
	local pond = FindEntity(inst, 20, function(inst) return inst.components.fishable.fishleft > 0 end, 
	FISHABLE_TAGS, FISHABLE_NO_TAGS)

	return pond and not inst.sg:HasStateTag("fishing") and inst.CanFish and BufferedAction(inst, pond, ACTIONS.FISH)
end

local function IsHomeOnFire(inst)
	local home = inst.components.homeseeker ~= nil and inst.components.homeseeker.home or nil
	return home and home.components.burnable and home.components.burnable:IsBurning() and home:IsNear(inst, SEE_HOME_DIST)
end
...
local function GetTraderFn(inst)
	...
	for _, v in ipairs(players) do
		if inst.components.trader ~= nil and inst.components.trader:IsTryingToTradeWithMe(v)
		and not inst.sg:HasStateTag("fishing") then
			return v
		end
	end
end
...
local function GetFaceTargetFn(inst)
	if inst.components.timer ~= nil and inst.components.timer:TimerExists("dontfacetime") then
		return nil
	end

	local shouldface = inst.components.follower ~= nil and inst.components.follower.leader
	or FindClosestPlayerToInst(inst, SEE_PLAYER_DIST, true)

	if shouldface and inst.components.timer ~= nil and not inst.components.timer:TimerExists("facetime") then
...
local function KeepFaceTargetFn(inst, target)
	...
	local keepface = (inst.components.follower ~= nil and inst.components.follower.leader
	and inst.components.follower.leader == target) or (target:IsValid() and inst:IsNear(target, SEE_PLAYER_DIST))
...
function MeadowMermFisherBrain:OnStart()
