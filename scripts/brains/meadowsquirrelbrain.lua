...
local function GoHomeAction(inst)
	...
	inst.components.homeseeker:HasHome() then
		if inst.components.homeseeker.home:HasTag("stump")
		or inst.components.homeseeker.home:HasTag("burnt")
		or inst.components.homeseeker.home:HasTag("fire") then
...
local function EatFoodAction(inst)
	local target = FindEntity(inst, SEE_BAIT_DIST, function(item)
		return inst.components.eater:CanEat(item) and item.components.bait and not item:HasTag("planted")
...
local function CheckForHome(inst)
	if not inst.components.homeseeker then
		if not inst.findhometask then
			inst.findhometask = inst:DoPeriodicTask(30, function()
				FindHome(inst) 
			end)
		end
