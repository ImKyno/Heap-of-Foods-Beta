...
local WobsterMonkeyIslandBrain = Class(Brain, function(self, inst)
...
local function GetStruggleDirection(inst)
	local rod = inst.components.oceanfishable:GetRod()
...
local function GetTiredOutDirection(inst)
	local rod = inst.components.oceanfishable:GetRod()
...
local function GetInstanceLureTarget(inst)
	return (inst._lure_target ~= nil and inst._lure_target:IsValid() and inst._lure_target) or nil
end

local function GetInstanceLureTargetPosition(inst)
	return (inst._lure_target ~= nil and inst._lure_target:IsValid() and inst._lure_target:GetPosition()) or nil
end
...
local function FindLureTarget(inst)
	if GetInstanceLureTarget(inst) == nil then
		inst._lure_target = FindEntity(inst, SEE_LURE_DISTANCE, function(item)
			return item.components.oceanfishinghook ~= nil and TheWorld.Map:IsOceanAtPoint(item.Transform:GetWorldPosition())
			end, LURE_MUST_TAGS, LURE_MUST_NOT_TAGS)
...
local function NibbleLure(inst)
	local action_to_perform = nil
	local lure = GetInstanceLureTarget(inst)
...
function WobsterMonkeyIslandBrain:OnStart()
					...
					PriorityNode({
						...
						PriorityNode({
                            WhileNode(struggle_update, "Struggling",
								Wander(self.inst, GetFisherPosition, TUNING.OCEAN_FISHING.MAX_HOOK_DIST, STRUGGLE_WANDER_TIMES, GetStruggleDirection, nil, nil, STRUGGLE_WANDER_DATA)
							),
							Wander(self.inst, GetFisherPosition, TUNING.OCEAN_FISHING.MAX_HOOK_DIST, TIREDOUT_WANDER_TIMES, GetTiredOutDirection, nil, nil, TIREDOUT_WANDER_DATA),
						}),
					})
				),

				WhileNode(should_go_home, "Should Go Home",
					DoAction(self.inst, GoHomeAction, "Going Home")
				),

				NotDecorator(
					ActionNode(function() FindLureTarget(self.inst) end)
				),
                WhileNode(
					function()
						return GetInstanceLureTarget(self.inst) ~= nil
					end,
					"Lure Nibbling",
					LoopNode {
						ParallelNodeAny {
							WaitNode(function() return 2 + math.random() end),
							Wander(self.inst, GetInstanceLureTargetPosition, LURE_WANDER_DIST, LURE_WANDER_TIMES, nil, nil, nil, LURE_WANDER_DATA),
						},
						DoAction(self.inst, NibbleLure),
						ConditionWaitNode(function() return self.inst:GetBufferedAction() == nil end),
					}
				),

				Wander(self.inst, GetHomeLocation, MAX_WANDER_DISTANCE, WANDER_TIMES),
			}, 0.30)
		),
	}, 0.30)
...
return WobsterMonkeyIslandBrain