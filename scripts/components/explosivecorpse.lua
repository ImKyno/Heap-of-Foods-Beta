...
local ExplosiveCorpse = Class(function(self,inst)
	self.inst = inst
...
function ExplosiveCorpse:SetOnExplodeFn(fn)
	self.onexplodefn = fn
end

function ExplosiveCorpse:SetAttacker(attacker)
	self.attacker = attacker
end

function ExplosiveCorpse:SetPvpAttacker(attacker)
	self.pvpattacker = attacker
end

local CANT_TAGS = { "INLIMBO" }

function ExplosiveCorpse:OnBurnt()
	...
	for i, v in ipairs(ents) do
		if v ~= self.inst and not v:IsInLimbo() and v:IsValid() and
			(self.pvpattacker == nil or v == self.pvpattacker or not v:HasTag("player")) then
			...
			if not v:IsInLimbo() and v:IsValid() then
				...
				if v.components.combat ~= nil and not (v.components.health ~= nil and v.components.health:IsDead()) then
					...
					if v.components.explosiveresist ~= nil then
						...
						v.components.explosiveresist:OnExplosiveCorpseDamage(dmg, self.inst)
					end
	...
	TheWorld:PushEvent("explosion_heard", Vector3(x, y, z))
end

return ExplosiveCorpse