-- Copy of birdspawner but modified to spawn birds only during the night.
local SourceModifierList = require("util/sourcemodifierlist")
local easing = require("easing")

return Class(function(self, inst)
	assert(TheWorld.ismastersim, "NightBirdSpawner should not exist on client")

	local NIGHTBIRD_TYPES =
	{
		[WORLD_TILES.ROCKY]               = { "kyno_bird_robin_night" },
		[WORLD_TILES.DIRT]                = { "kyno_bird_robin_night" },
		[WORLD_TILES.SAVANNA]             = { "kyno_bird_robin_night" },
		[WORLD_TILES.GRASS]               = { "kyno_bird_robin_night" },
		[WORLD_TILES.FOREST]              = { "kyno_bird_robin_night" },
		[WORLD_TILES.MARSH]               = { "kyno_bird_robin_night" },

		-- We don't have an ocean bird yet.
	...
	-- Custom TUNING in case people want to mess with nightbirds only.
	local _maxbirds                 = TUNING.KYNO_NIGHTBIRD_SPAWN_MAX
	local _minspawndelay            = TUNING.KYNO_NIGHTBIRD_SPAWN_DELAY.MIN
	local _maxspawndelay            = TUNING.KYNO_NIGHTBIRD_SPAWN_DELAY.MAX

	-- These can use default TUNING.
	...
	local BIRD_MUST_TAGS            = { "nightbird" }
	...
	local function GetPostHailEasingMult()
		return inst.components.timer:TimerExists(HAIL_EVENT_TIMERS.POST_HAIL)
		and easing.inQuad(inst.components.timer:GetTimeElapsed(HAIL_EVENT_TIMERS.POST_HAIL), 0, 1, _posthail_time) or 1
	end

	local function UpdatePostHailModifier()
		if inst.components.timer:TimerExists(HAIL_EVENT_TIMERS.POST_HAIL) then
			_timescale_modifiers:SetModifier(inst, TUNING.BIRD_POST_HAIL_FACTOR 
			- (TUNING.BIRD_POST_HAIL_FACTOR * GetPostHailEasingMult()), SPAWN_FACTOR_KEYS.POST_HAIL)
		else
	...
	local function SpawnCorpseForPlayer(player, reschedule)
		...
		if reschedule then
			...
			if corpse then
				if CanLunarPreRiftMutateFromCorpse(corpse)
				or TryLuckRoll(player, TUNING.LUNARHAIL_MUTATE_BIRD_CHANCE, LuckFormulas.PreRiftMutation) then
	...
	local function SpawnBirdForPlayer(player, reschedule)
		...
		if bird_count < CalcValue(player, _maxbirds, "maxbirds") then
			...
			if spawnpoint ~= nil then
				self:SpawnBird(spawnpoint) -- DON'T RETURN!
			end
		end
	...
	local function ScheduleSpawn(player, initialspawn)
		...
		if _scheduledtasks[player] == nil then
			...
			_scheduledtasks[player] = player:DoTaskInTime(GetRandomMinMax(lowerbound, upperbound)
			* _timescale_modifiers:Get(), GetBirdSpawnerFunction(), ScheduleSpawn)
		end
	end
	...
	local function CanBirdsSpawn()
		return _worldstate.isnight and _maxbirds > 0
	end
	...
	local function GetMutatedBirdSpawnChance(spawnpoint)
	...
	local function PickBird(spawnpoint)
		local bird = "kyno_bird_robin_night"
		local tile = _map:GetTileAtPoint(spawnpoint:Get())

		if NIGHTBIRD_TYPES[tile] ~= nil then
			bird = GetRandomItem(NIGHTBIRD_TYPES[tile])
		end

		if IsSpecialEventActive(SPECIAL_EVENTS.CARNIVAL) and bird ~= "kyno_bird_robin_night" and IsLandTile(tile) then
			...
			if TheSim:CountEntities(x, y, z, TUNING.BIRD_CANARY_LURE_DISTANCE, nil, nil, CARNIVAL_EVENT_ONEOF_TAGS) > 0 then
				bird = "kyno_bird_robin_night"
			end
		elseif bird == "kyno_bird_robin_night" then
			local x, y, z = spawnpoint:Get()

			if TheSim:CountEntities(x, y, z, TUNING.BIRD_CANARY_LURE_DISTANCE, SCARECROW_TAGS) > 0 then
				bird = "kyno_bird_robin_blue_night"
			end
		end

		if TUNING.SPAWN_MUTATED_BIRDS_GESTALT and math.random() < GetMutatedBirdSpawnChance(spawnpoint) and bird ~= "puffin" then
			bird = "mutatedbird"
		end

		-- In winter, it has a chance to replace robin_night.
		return _worldstate.iswinter and bird == "kyno_bird_robin_night"
		and math.random() < TUNING.KYNO_NIGHTBIRD_ROBIN_NIGHT_WINTER_CHANCE and "kyno_bird_robin_winter_night" or bird
	end

	local function IsDangerNearby(x, y, z)
		return TheSim:CountEntities(x, y, z, 8, SCARYTOPREY_TAGS) > 0
	end
		...
		for _, player in ipairs(_activeplayers) do
			...
			local function SpawnBirdCorpse(_, mutate)
				...
				if corpse then
					...
					elseif CanLunarPreRiftMutateFromCorpse(corpse)
					or TryLuckRoll(player, TUNING.LUNARHAIL_MUTATE_BIRD_CHANCE, LuckFormulas.PreRiftMutation) then
						corpse:SetNonGestaltCorpse()
					else
						corpse:StartFadeTimer(GetRandomMinMax(_corpse_fade_min_time, _corpse_fade_max_time))
			...
			-- This should ALWAYS happen at least after one corpse has spawned.
			inst:DoTaskInTime(2 + 3 * math.random(), AnnounceCorpses)
		end
	end
	...
	local function OnIsLunarHailing(inst, ishailing, onpostinit)
		if ishailing then
			local bird_event_time = TUNING.LUNARHAIL_EVENT_TIME
			* GetRandomWithVariance(TUNING.LUNARHAIL_BIRD_EVENT, TUNING.LUNARHAIL_BIRD_EVENT_VARIANCE)

			-- OnIsLunarHailing runs on load and timers already save.
			if not inst.components.timer:TimerExists(HAIL_EVENT_TIMERS.CORPSES) and not onpostinit then
			...
			if _ishailing then -- It was hailing, now it's ended! The bird population has been destroyed!
	...
	function self:GetSpawnPoint(pt, is_corpse)
		local function TestSpawnPoint(offset)
			...
			local allow_water = false -- Don't allow water until we have a bird for it.
			local in_moonstorm = TheWorld.net.components.moonstorms
			and TheWorld.net.components.moonstorms:IsXZInMoonstorm(spawnpoint_x, spawnpoint_z)
			...
			and (is_corpse or (not in_moonstorm and not _groundcreep:OnCreep(spawnpoint_x, spawnpoint_y, spawnpoint_z)))
		end
	...
	{
		default                      = "kyno_bird_robin_night",
		kyno_bird_robin_winter_night = "kyno_bird_robin_winter_night",
		kyno_bird_robin_blue_night   = "kyno_bird_robin_blue_night",
	}
	...
	function self:SpawnBird(spawnpoint, ignorebait)
		...
		if bird.components.eater ~= nil and not ignorebait then
			local bait = TheSim:FindEntities(spawnpoint.x, 0, spawnpoint.z, 15, nil, BAIT_CANT_TAGS)

			for k, v in pairs(bait) do
				local x, y, z = v.Transform:GetWorldPosition()

				if bird ~= nil and bird.components.eater ~= nil and bird.components.eater:CanEat(v) and not v:IsInLimbo()
				and v.components.bait ~= nil and not (v.components.inventoryitem ~= nil and v.components.inventoryitem:IsHeld())
				and not IsDangerNearby(x, y, z) and (bird.components.floater ~= nil or _map:IsPassableAtPoint(x, y, z)) then
					spawnpoint.x, spawnpoint.z = x, z
					bird.bufferedaction = BufferedAction(bird, v, ACTIONS.EAT)
					found_bait = true
					break
				elseif v.components.trap ~= nil and v.components.trap.isset
				and (not v.components.trap.targettag or bird:HasTag(v.components.trap.targettag))
				and not v.components.trap.issprung and math.random() < TUNING.BIRD_TRAP_CHANCE and not IsDangerNearby(x, y, z) then
					spawnpoint.x, spawnpoint.z = x, z
					found_bait = true
					break
				end
	...
	-- Mods.
	function self:SetBirdTypesForTile(tile_id, bird_list)
		NIGHTBIRD_TYPES[tile_id] = bird_list -- Don't make me regret giving you access!
	end

	-- Mods.
	function self:SetTimeScaleModifier(factor, key)
	...
	-- Wickerbottom's book.
	function self:SpawnCorpseForPlayer(player)
	...
	function self:GetDebugString()
		...
		return string.format("birds:%d/%d, time scale modifier:%.2f, post hail easing mult: %.3f",
		numbirds, _maxbirds, _timescale_modifiers:Get(), GetPostHailEasingMult())
	end
end)