...
-- Launch at random position if launcher is nil.
function LaunchAtRandomly(inst, launcher, basespeed)
	if inst ~= nil and inst.Physics ~= nil and inst.Physics:IsActive() then
		local x, y, z = inst.Transform:GetWorldPosition()
		local vx, vz

		if launcher ~= nil and launcher.Transform ~= nil then
			local x1, y1, z1 = launcher.Transform:GetWorldPosition()
			vx, vz = x - x1, z - z1
		else
			local angle = math.random() * TWOPI
			local spd = (basespeed or 5) + math.random() * 2
		...
		local angle = spd > 0 and math.atan2(vz / spd, vx / spd) + (math.random() * 20 - 10) * DEGREES or math.random() * TWOPI
