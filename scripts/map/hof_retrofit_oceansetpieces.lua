...
local function HofRetrofitting_OceanSetpieces(map, savedata)
	local obj_layout = require("map/object_layout")
	...
	for i, layout in ipairs(hof_ocean_setpieces) do
		local success = TryToAddLayout(layout, 5)
	end	
	
	if success then
		print("Retrofitting for Heap of Foods Mod - Added the Ocean Setpieces to the world.")
	else
		print("Retrofitting for Heap of Foods Mod - Failed to add the Ocean Setpieces to the world!")
	end
end
