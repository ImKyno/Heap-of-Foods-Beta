...
local function ShouldAcceptItem(inst, item)
    ...
    if item:HasTag("oceanfish") or
       ...
       item.prefab == "hermit_cracked_pearl" or item.prefab == "meat"
        then
        return true
    end
end
...
local function GiveCannedFood(inst)
	local cannedloot = SpawnPrefab(math.random() < 0.5 and "kyno_tomatocan" or "kyno_beancan")
	local pt = Vector3(inst.Transform:GetWorldPosition()) + Vector3(0,2,0)
	cannedloot.Transform:SetPosition(pt:Get())
	local down = TheCamera:GetDownVec()
	local angle = math.atan2(down.z, down.x) + (math.random()*60)*DEGREES
	local sp = 3 + math.random()
	cannedloot.Physics:SetVel(sp*math.cos(angle), math.random()*2+8, sp*math.sin(angle))
end

local function GiveCannedMeat(inst)
	local cannedloot = SpawnPrefab("kyno_meatcan")
	local pt = Vector3(inst.Transform:GetWorldPosition()) + Vector3(0,2,0)
	cannedloot.Transform:SetPosition(pt:Get())
	local down = TheCamera:GetDownVec()
	local angle = math.atan2(down.z, down.x) + (math.random()*60)*DEGREES
...
local function initfriendlevellisteners(inst)
    ...
    inst:ListenForEvent("itemget", function(world,data)
        ...
        if data.item:HasTag("oceanfish") then
            ...
            if data.item.components.weighable:GetWeightPercent() >= TUNING.HERMITCRAB.HEAVY_FISH_THRESHHOLD then
                ...
                if data.item.prefab == "oceanfish_small_6_inv"  then
					...
					dospecialfish(TASKS.GIVE_BIG_AUTUM, "kyno_tunacan")
        ...
        elseif data.item.prefab == "flowersalad" then
			...
			--------------------------------------------------------------------------------------
			inst:DoTaskInTime(1.1, function() GiveCannedFood(inst) end)
			--------------------------------------------------------------------------------------
        ...
        elseif data.item.components.edible then
			...
			--------------------------------------------------------------------------------------
			if data.item.prefab == "meat" then
				inst:DoTaskInTime(1.1, function() GiveCannedMeat(inst) end)
			end
			--------------------------------------------------------------------------------------
            inst:PushEvent("eat_food")
            data.item:Remove()
