...
local function OnUpdateThorns(inst)
	...
	for i, v in ipairs(TheSim:FindEntities(x, y, z, inst.range + 3, COMBAT_TARGET_TAGS, inst.canhitplayers and NO_TAGS or NO_TAGS_NO_PLAYERS)) do
		...
		and not (v.components.inventory ~= nil and v.components.inventory:EquipHasTag("bramble_resistant"))
		and not (v.components.debuffable ~= nil and v.components.debuffable:HasDebuff("kyno_thornsbuff")) then
			...
			if v:GetDistanceSqToPoint(x, y, z) < range * range then
				...
				if inst.owner ~= nil then
					...
					and not inst.owner.components.combat:IsAlly(v) then
						inst.ignore[v] = true
						v.components.combat:GetAttacked(v.components.follower and v.components.follower:GetLeader() == inst.owner
						and inst or inst.owner, inst.damage, nil, nil, inst.spdmg)
					end
				elseif v.components.combat:CanBeAttacked() then
					...
					if not inst.canhitplayers then
						...
						isally = leader ~= nil and leader:HasTag("player")
						and not (v.components.combat ~= nil and v.components.combat.target ~= nil
...
return MakeFX("kyno_bramblefx", "idle", "ARMORBRAMBLE_DMG")