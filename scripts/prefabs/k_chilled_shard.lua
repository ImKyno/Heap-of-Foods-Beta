local assets =
{
	Asset("ANIM", "anim/kyno_chilled_shard.zip"),
}
...
local function OnUpdateFlock(inst)
	...
	inst.range = inst.range + TUNING.KYNO_CHILLED_SHARD_RANGE
...
local function fn(planardamage)
	...
	inst.AnimState:SetBank("kyno_chilled_shard")
	inst.AnimState:SetBuild("kyno_chilled_shard")
	...
	inst.damage = TUNING.KYNO_CHILLED_SHARD_DAMAGE
	inst.spdmg = nil -- planardamage and { planar = TUNING.KYNO_CHILLED_SHARD_DAMAGE_PLANAR } or nil
	inst.range = TUNING.KYNO_CHILLED_SHARD_RANGE
...
return Prefab("kyno_chilled_shard", fn, assets)