local hof_hermitcrabtea_defs = require("prefabs/k_hermitcrabtea_defs")
local HOF_TEA_DEFS = hof_hermitcrabtea_defs.teas
local HOF_BUFF_DEFS = hof_hermitcrabtea_defs.buffs

hof_hermitcrabtea_defs = nil
...
local function OnFinishTea(inst)
	...
	if owner then
		...
		if feeder and feeder:IsValid() and
			feeder.components.inventory and feeder.components.inventory.isopen and feeder:GetDistanceSqToPoint(x, y, z) < range * range then
...
local function MakeTea(data)
	local prefabname = "kyno_hermitcrabtea_"..data.name

	local overridesym_build = data.build or "kyno_hermitcrab_tea"
	local overridesym = "tea_bottle_"..data.name

	local assets =
	{
        Asset("ANIM", "anim/kyno_hermitcrab_tea.zip"),
		Asset("SCRIPT", "scripts/prefabs/k_hermitcrabtea_defs.lua"),
	...
	local healthvalue = data.healthvalue or 0
	...
	local function fn()
		...
		inst.AnimState:SetBuild("kyno_hermitcrab_tea")
...
local hof_tea_prefabs = {}

for _, data in ipairs(HOF_TEA_DEFS) do
	if not data.data_only then
		table.insert(hof_tea_prefabs, MakeTea(data))
	end
end

for _, data in ipairs(HOF_BUFF_DEFS) do
	if not data.data_only then
		table.insert(hof_tea_prefabs, MakeTeaBuff(data))
	end
end

return unpack(hof_tea_prefabs)