local assets =
{
	Asset("ANIM", "anim/kyno_tomato_trail.zip"),
}
...
local function OnUpdate(inst, x, y, z, rad)
	for i, v in ipairs(TheSim:FindEntities(x, y, z, rad, SLOWDOWN_MUST_TAGS, SLOWDOWN_CANT_TAGS)) do
		if v.components.locomotor ~= nil then
			v.components.locomotor:PushTempGroundSpeedMultiplier(TUNING.KYNO_TOMATO_TRAIL_SPEED, WORLD_TILES.MUD)
		end
	end
end
...
local function OnUpdateClient(inst, x, y, z, rad)
	...
	if player ~= nil and player.components.locomotor ~= nil and not player:HasTag("playerghost")
	and player:GetDistanceSqToPoint(x, 0, z) < rad * rad then
		player.components.locomotor:PushTempGroundSpeedMultiplier(TUNING.KYNO_TOMATO_TRAIL_SPEED, WORLD_TILES.MUD)
	end
end
...
local function fn()
	...
	inst.AnimState:SetBank("kyno_tomato_trail")
	inst.AnimState:SetBuild("kyno_tomato_trail")
	inst.AnimState:SetLayer(LAYER_BACKGROUND)
	inst.AnimState:SetSortOrder(3)

	inst._isfading = net_bool(inst.GUID, "tomato_trail._isfading", "isfadingdirty")
...
return Prefab("kyno_tomato_trail", fn, assets)